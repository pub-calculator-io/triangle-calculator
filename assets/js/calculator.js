function calculate(){
  ...
  const check = values => values.every(item=>item); 
  ...
  input.silent = false;
  if(check([A,B,C]) || [a,b,c,A,B,C].reduce((count,item)=>item?count+1:count,0) != 3){
    input.error([],"Please provide 3 positive values, including at least one side");
  }
  if(check([A,B])&&(A+B>=180) || check([B,C])&&(B+C>=180) || check([A,C])&&(A+C>=180)){
